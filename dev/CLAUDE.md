...
- Render is a function of the hash: `route()` rebuilds the listing only when the `shown` key (file, event, scale) changes — a rebuild of the same file keeps `scrollTop`, a new file without `l` centers its hottest line — then `detailSet(line)` reconciles the popup and `tr.target` (`detailOpen()`; the row is centered only if `rowOnScreen()` is false, so a clicked row stays put while a chip/link/bookmark scrolls to it). Scale change: `shown = ""` then `route()`; event pick: `location.hash = hashOf({...st, ev})`.
