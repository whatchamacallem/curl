...
- Every table is `theme.table()` / heat map's `table()`: draggable resize bar at every column boundary including the trailing edge. Column widths are **never persisted** — reload always resets to default; "reset columns" link (top-level strip only) undoes drags for the current view only. `fill` tables (summary functions table, heat map's per-file source listing, cgdiff table) open at 90% of the visible width of the pane that scrolls them (`scrollerOf()` in theme.js: nearest `overflow: auto|scroll` ancestor, else the document — never `window.innerWidth`, the heat map's listing pane is only part of the window), floored to whole px so rounding can't raise a sideways scrollbar; the source listing's source column fills the rest. Computed once at first render; dragging never re-triggers that calc. Heat map's two home tables ("Hottest lines by"/"Hottest functions by") are plain (non-`fill`) tables sized to content, same as the tree pane, so a long column header can't stretch a heat-colored value cell into a wide bar or push a trailing column off-screen. No per-column tooltip is a "legend" link anymore — glossary lives once in README.md. Full row is a click target on `fill`/rowlink tables; only the one real `<a>` cell shows hover state.
