...
MANIFEST.txt        line 1 = the generator's version string, `curl/perf2html.sh v1` —
                    ...
                    cpu / build / executable — `sample_meta()` in perf2html.sh sets
...
- `build_report.py test FILE -o OUT --test TITLE --raw-data FILE [--log FILE] [--diff] [--header LABEL=VALUE]...` — the summary page; `--diff` dispatches to `BuildReport.diff_test` (abs ranking, signed numbers, `DIFF_VIEWS` strip, no calls/callers columns); `--header` rows form the page's top table. `build_report.py timing -o OUT --test NAME --output-file FILE [--header ...]` — the native timing page. `build_report.py overview -o OUT --test NAME... [--header LABEL=VALUE] [--header-file FILE] [--header-block LABEL=FILE] [--diff]` — the cross-test page; `--header-file` appends a file's `LABEL=VALUE` lines to the `--header` rows and silently drops every other line (that is how `MANIFEST.txt` reaches the page, version line and all), `--header-block` adds a further table under its own `<h2>` from such a file (repeatable; the diff's baseline/modified blocks), `--diff` swaps each test's native timing for its `Ir` change. **"header" = the label/value rows above a page's real content** (`Header`/`HeaderBlock`, `BuildReport.header_table`/`.header_read_file`/`.header_parse_blocks`) — not the heat map's `MetaModel`, which is a profile's event metadata. `EVENT = "Ir"` and `TOP = 50` are constants.
