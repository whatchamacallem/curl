...
- Writes `OUTDIR/MANIFEST.txt` = `curl/perf2html.sh v1\n`, nothing else. It is what marks a directory as a usable `perf2html_diff.sh` input; `validate_report.py` requires it. A report made before this existed fails both — regenerate it.
...
```sh
dev/perf2html_diff.sh ~/before ~/after            # -> dev/report-diff
dev/perf2html_diff.sh ~/before ~/after ~/delta    # chosen OUTDIR
DIFF_EVENT=CEst dev/perf2html_diff.sh ~/b ~/a     # rank by another event (default Ir)
```

Full reference: `dev/perf2html_diff.md` / `--help`. Key facts:
- Inputs are two `perf2html.sh` OUTDIRs; it reads their `*/raw/callgrind.out.*` (plus a single-test report's top-level `raw/`). Tests are matched by the directory name the `raw/` sits under and intersected — `all` vs `all`, the rest vs the rest, no overlap logic. A one-sided test is noted on stderr and skipped.
- Manifest gate both ways: an input must be exactly the newline-terminated line `curl/perf2html.sh v1` (`cmp` against `printf '%s\n'`); a missing file, extra/unknown content or a missing final newline is refused. Its own OUTDIR gets `curl/perf2html_diff.sh v1`, and an input carrying that is refused with "A diff cannot be used to make a diff."
- Output per test: `index.html` + `heat-map/` + `raw/callgrind.diff.<test>.<ts>` only. **No flame graph, no native timing** — a delta has no call graph for speedscope, and two wall clocks don't subtract. `validate_report.py` enforces their absence on a diff.
- Every number is CURRENT − BASELINE, **per source line**: `callgrind_diff.py` parses both sides with the shared parser, subtracts per (function, file, line) via `Profile.function_lines` — never per (file, line) alone, which hands roughly half of an inlined function's cost to its neighbour (`line_function` is first-seen) — and writes a plain callgrind-format file: `fl=`/`fn=`/`fi=` per function, that function's entry line first (a zero-cost line if it did not change), `summary:` = the signed total so the 1.0000 self-check holds on the delta too. No `calls=` lines: the delta has no call graph, hence no calls/callers columns in the summary and no call columns/caller tables in the heat map (`HAS_CALLS`).
...
- Delta header: `desc: Baseline: <files>`, `desc: Current: <files>`, `desc: Baseline totals: <name> <value> ...`; `callgrind_diff.profile_baseline_total()` parses the last through `profile.value()`, so the overview's "% of baseline" works for derived events too. Written paths have `--repo-root` stripped.
- `dev/trace/callgrind.diff.<test>.<ts>` + `diff.<ts>.log` (quiet mode).

**Core vs diff code.** The non-diff path is untouched and byte-for-byte checked: a regular profile's summary page matches the pre-diff generator exactly, the heat map differs only in script text and a `meta.diff:false` key (build both from one `raw/` file with `git show HEAD:dev/scripts/*` copies and `cmp`). `build_report.py`: `report_test`/`report_overview`/`functions_table` are core, `diff_report_test`/`diff_report_overview`/`diff_functions_table`/`diff_overview_rows` are the diff, both through the assemblers `report_page()`/`overview_page()`/`test_profile()`; `--diff` only picks the function in `main()`. `callgrind_to_heatmap.py`: `model_build()` is core, `diff_model()` patches `meta.totals`/`meta.diff`; in its JS every diff override sits in the one `if (DIFF) {...}` block (`fmtP`/`fmtH` wrapped with a sign, `HOT`/`HOT_LINES`/`SELF.title`), and ranking/heat go through `mag()` (= `Math.abs`, a no-op on a profile's non-negative costs). `validate_report.py` is data-driven by `Layout` (`LAYOUT_FULL`/`LAYOUT_DIFF`, picked by the manifest).
...
- `build_report.py` — `test`/`timing`/`overview` pages. `--diff` dispatches `test`/`overview` to `diff_report_test`/`diff_report_overview` (abs ranking, signed numbers, `DIFF_VIEWS` strip, no calls/callers columns); `--meta LABEL=VALUE` (repeatable) adds rows to the summary's "what this is" table; `--event` picks the overview's ranking event.
- `callgrind_diff.py` — parses both sides with the shared parser, subtracts per (function, file, line), writes a callgrind-format delta (see perf2html_diff.sh above). `--baseline`/`--current` repeatable (merged per side), `-o`, `--repo-root`. Prints the must-be-1.0000 self-check ratio per side. Also home of `profile_magnitudes()`/`profile_baseline_total()`, which the generators import.
- `callgrind_to_heatmap.py` — heat-map/index.html. `--diff` = `diff_model()`: shares against `profile_magnitudes()`, signed numbers, "Most changed" headings (the JS `if (DIFF)` block). Call columns and the popup's caller table/note exist only when the profile has a call graph (`HAS_CALLS`).
- `callgrind_to_speedscope.py` (`--event` repeatable, `A+B` sums, `--repo-root` relativizes) + `build_flame_graph.py` — flame-graph/.
- `validate_report.py OUTDIR` — structural smoke test (pages exist/non-truncated, strip links present, `raw/` has real files, timing line present, no leaked absolute paths). `--test NAME` narrows an overview check. Reads `MANIFEST.txt` first and picks `LAYOUT_FULL`/`LAYOUT_DIFF` by exact newline-terminated match (anything else fails and checks as full): the diff layout is heat map only, "by change in self" heading, flame-graph/ and perf-tool/ must be *absent*, no `all` link required.
