...
Typing rules in `dev/scripts/` (everything annotated, no `Any`-shaped records): a record is a `NamedTuple` (`callgrind.LineKey`/`Term`/`DerivedEvent`, `theme.Col`/`Cell`/`Pair`, `build_report.View`/`StripLink`, heat map `LineCost`/`CallRow`); anything summed in place is a `@dataclass` (`Profile`, `CallCost`, speedscope `Graph`, heat map `_FileAcc`/`_LineAcc`). JSON-bound shapes: an object is a `TypedDict` (`HeatModel`/`FileModel`/`FunctionModel`, `SpeedscopeDoc`, `ThemeRuntime` — a NamedTuple would serialize as an array), a positional array is a NamedTuple (`json.dumps` writes it as the `[a, b, c]` the page indexes). Cost vectors are `callgrind.Vec` (`list[int]`), summed only via `vec_add`/`vec_acc`/`call_acc`. Each CLI turns argparse into a NamedTuple (`TestArgs`, `HeatArgs`, `SpeedscopeArgs`, ...) before calling anything. Naming: a field that would shadow a method of its base class gets a trailing underscore, never a synonym — a NamedTuple's `index`/`count` (pyright: overrides `tuple`'s methods) is `index_`/`count_` (`IndexedTerm.index_`, `LineCost.count_`); a dataclass field with the same meaning stays plain (`CallCost.count`). Generator output is reproducible run to run (the heat map's `model_build` walks raw file names sorted, so the `raw` field of an external header that several build paths map to is always the same one); byte-compare generated pages directly.
