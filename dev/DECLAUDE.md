...
### `dev/perf2html.sh` — build + profile + report (urlparser only)

```sh
dev/perf2html.sh                                     # generates `diff2html_baseline_report`.
dev/perf2html.sh -DCMAKE_C_FLAGS="-O3 -march=native" # generates `diff2html_modified_report`.
...
- cwd-independent (cd's to `dev/`): a relative `--report=DIR` is `dev/DIR`, `~/` is expanded. Default DIR is `diff2html_baseline_report`, or `diff2html_modified_report` when any cmake_flags are given — after a *source* change alone, pass `--report=diff2html_modified_report` yourself. Last line printed is the report's `file://` URL.
...
- No env vars — constants at the top of the script: `TEST=urlparser`, `LOOPS=200`, `CPU=3`, `BUILD_DIR`. Extra valgrind flags (LL cache auto-detects as direct-mapped and overstates conflict misses; `--LL=16777216,16,64` fixes it) go on the `valgrind` line in `profile_run`.
...
- `validate_report.py OUTDIR` runs last (structural smoke test, not a numeric re-check); a run that finishes without error has every expected page.
...
index.html          strip (title badge + "|"-separated links) + summary: collapsed
                    valgrind log, collapsed raw-data link, "top 50 functions by self"
                    table (#, % self, symbol, calls, callers — callers/symbol link to
                    heat map)
flame-graph/        speedscope bundle + profile.js (Ir, D1mr+D1mw, DLmr+DLmw, I1mr, Bcm, Bim)
heat-map/           per-line source heat map (event selector, miss columns)
perf-tool/          native timing run output (output.txt + index.html with binary/build meta)
README.md           event-column glossary + flame graph / heat map notes; copied from
                    dev/README.md every run; opened by the "help" link
raw/                the callgrind file, repo-root path prefix stripped
```

Raw data in `dev/trace/` (gitignored): `callgrind.out.urlparser.200.<ts>`, `valgrind.urlparser.200.<ts>.log`, `urlparser.200.<ts>.speedscope.json`, `profile.<ts>.log` (quiet mode only). Every path embedded in a generated page is relative — OUTDIR is portable.

### `dev/perf2html_diff.sh` — compares two reports

```sh
dev/perf2html_diff.sh                             # `diff2html_baseline_report` vs `diff2html_modified_report` -> `diff2html_diff_report`
dev/perf2html_diff.sh ~/before ~/after            # -> `diff2html_diff_report`
...
- Inputs are two `perf2html.sh` OUTDIRs, each holding exactly one `raw/callgrind.out.*` (a diff report holds `raw/callgrind.diff.*`, so a diff can't be diffed; there is no manifest). The test name comes from that file name; the page title is `urlparser diff`.
- Output: `index.html` + `heat-map/` + `raw/callgrind.diff.<test>.<ts>` + `README.md`. **No flame graph, no native timing** — a delta has no call graph for speedscope, and two wall clocks don't subtract.
...
Every CLI below is invoked only by the two shell scripts; `dev/api.md` is the checklist of what each still takes.
...
- `build_report.py test FILE -o OUT --test TITLE --raw-data FILE [--log FILE] [--diff] [--meta LABEL=VALUE]...` — the summary page; `--diff` dispatches to `diff_report_test` (abs ranking, signed numbers, `DIFF_VIEWS` strip, no calls/callers columns); `--meta` rows form the page's top table. `build_report.py timing -o OUT --test NAME --output-file FILE [--meta ...]` — the native timing page. `EVENT = "Ir"` and `TOP = 50` are constants.
...
- `validate_report.py OUTDIR [--diff]` — structural smoke test (pages exist/non-truncated, titles, strip links, `raw/` has a callgrind file without the absolute repo root, timing line + `Errors:` present). `--diff` = heat map only, "by change in self" heading.
...
`dev/lint.sh` (no args) runs pyright over `dev/scripts` per `dev/pyrightconfig.json` (`standard` mode, `pythonVersion` 3.11; must stay at **0 errors**) and `check_js.py` (catches a `\n` that needed `\\n` in JS-inside-a-Python-string before it reaches a generated page); exit 1 if either fails.
...
- Index page is a frame: strip (title badge + `|`-separated plain-word links, no brackets, `flex-wrap: nowrap` so it clips rather than wraps) + summary; sub-pages load into an iframe on click. Picked link/title highlighted at `--hot`/`--hot-fg`. Title = current view path (`urlparser / heat map`); there is no nesting (the summary page is the only frame page). "reset columns" / "help" / "curl.se/perf" sit right-aligned via `.strip .sp`.
...
"/mnt/c/Program Files/Google/Chrome/Application/chrome.exe" --headless=new --disable-gpu \
  ...
  "file://wsl.localhost/$WSL_DISTRO_NAME/home/t/curl/dev/diff2html_baseline_report/index.html#heat-map"
```
...
5. Compare: `dev/perf2html.sh --report=diff2html_modified_report` then `dev/perf2html_diff.sh`; keep only changes that measurably help AND keep `Errors:` constant.
