...
class BuildReport:
    def caller_delta_cell(self, profile: callgrind.Profile, deltas: Sequence[CallerDelta]) -> Cell:
        ...
        html_parts: list[str] = []
        for delta in deltas:
            count_text = f" {theme.num_signed(delta.count_)} calls" if delta.count_ else ""
            text = f"{theme.num_signed(delta.cost)} {delta.function}{count_text}"
            parts.append(text)
            ...
            html_parts.append(f'<a href="{href}">{label}</a>' if href else label)
        joined = ", ".join(parts)
...
def main() -> None:
    ...
    test_parser.add_argument("--callers-data", default="", metavar="FILE",
