...
class CallgrindDiff:
    ...
    def build(self, args: CallgrindDiff.DiffArgs) -> None:
        ...
        print(f"wrote {args.callers_output} "
    ...
    def callers_subtract(self, baseline: callgrind.Profile, modified: callgrind.Profile,
        ...
        for callee in sorted(set(baseline.callers) | set(modified.callers)):
            ...
            for caller_name in sorted(set(before) | set(after)):
                ...
                if count or cost:
                    deltas.append(CallgrindDiff.CallerDelta(caller_name, count, cost))
            if deltas:
                deltas.sort(key=lambda delta: (-abs(delta.cost), -abs(delta.count_),
                                               delta.function))
                out[callee] = deltas
        return out

    # Write the caller sidecar, as rows rather than objects to keep it small.
    def callers_write(self, callers: dict[str, list[CallgrindDiff.CallerDelta]],
                      path: str, event: str) -> None:
        doc: CallgrindDiff.CallersDoc = {
            "event": event,
            "callers": {callee: [[delta.function, delta.count_, delta.cost]
                                 for delta in deltas]
    ...
    def costs_sub(self, modified: Costs, baseline: Costs) -> Costs:
        return [(modified[index] if index < len(modified) else 0)
    ...
    def subtract(self, baseline: callgrind.Profile,
                 modified: callgrind.Profile) -> callgrind.Profile:
        ...
        diff = callgrind.Profile(events=list(modified.events),
        ...
        for function in sorted(set(baseline.function_lines) | set(modified.function_lines)):
            ...
            for key in sorted(set(before) | set(after)):
                costs = self.costs_sub(after.get(key, modified.zeros()),
    ...
    def write(self, profile: callgrind.Profile, path: str,
    ...
    def write_function(self, handle: TextIO, profile: callgrind.Profile, function: str,
                       current_ob: str) -> str:
        ...
        for file in [home] + sorted(name for name in by_file if name != home):
            ...
            if file == home:
                ...
                if entry is not None and entry.file == home and entry.line:
                    first = ([pair for pair in ordered if pair[0] == entry.line]
...
def main() -> None:
    ...
    parser.add_argument("--current", dest="modified", action="append", required=True,
                        metavar="FILE",
    ...
    CallgrindDiff().build(CallgrindDiff.DiffArgs(
        baseline=namespace.baseline, modified=namespace.modified,
        output=namespace.output, callers_output=namespace.callers_output, event=_EVENT))
