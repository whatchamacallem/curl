...
class Graph:
    ...
    def components(self) -> list[list[int]]:
        ...
        for root in range(node_count):
            ...
            stack.append(root)
            on_stack[root] = True
            work = [Graph._WalkFrame(root, iter(self.edges.get(root, {})))]
            while work:
                ...
                for neighbor in neighbors:
                    if discovery[neighbor] == -1:
                        ...
                        work.append(Graph._WalkFrame(neighbor, iter(self.edges.get(neighbor, {}))))
                        pushed = True
                        break
...
class CallgrindToSpeedscope:
    def document(self, profile: callgrind.Profile, name: str) -> SpeedscopeDoc:
        graph = self.graph(profile)
        if graph.cycles:
            print(f"collapsed {len(graph.cycles)} cycle(s) into one frame each: {'; '.join(graph.cycles)}",
                  file=sys.stderr)
        frames: list[Frame] = []
        ...
        for expr in EVENTS:
            indexes = self.expr_resolve(profile, expr)
            if indexes is None:
            ...
            if raw_total <= 0:
                ...
                continue
            label = self.expr_label(profile, expr)
            built = self.sampled_profile(graph, indexes, label)
    ...
    def expr_label(self, profile: callgrind.Profile, expr: str) -> str:
    ...
    def frame_file(self, home: str) -> str:
        return callgrind.path_norm(home).display if home and home != "???" else ""

    def graph(self, profile: callgrind.Profile) -> Graph:
        ...
        graph = Graph(event_count=len(profile.events), names=sorted_names,
                      files=[self.frame_file(profile.function_home[name]) for name in sorted_names],
                      index=index,
                      self_cost={index[name]: list(costs) for name, costs in profile.function_self.items()},
    ...
    def run(self, args: SpeedscopeArgs) -> None:
        profile = callgrind.profile_load(args.callgrind_file)
        doc = self.document(profile, args.name or os.path.basename(args.callgrind_file[0]))
        with open(args.output, "w", encoding="utf-8") as handle:
            json.dump(doc, handle)
        print(f"wrote {args.output} ({len(doc['shared']['frames'])} frames)", file=sys.stderr)

    def sampled_profile(self, graph: Graph, indexes: Sequence[int], name: str) -> BuiltProfile:
...
def main() -> None:
    ...
    CallgrindToSpeedscope().run(SpeedscopeArgs(callgrind_file=namespace.callgrind_file, output=namespace.output,
                                               name=namespace.name))
