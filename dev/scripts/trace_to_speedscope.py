...
SpeedscopeDoc = TypedDict("SpeedscopeDoc", {
    "$schema": str, "shared": Shared, "profiles": list[EventedProfile], "name": str,
    "exporter": str})
...
class TraceToSpeedscope:
    ...
    def calls(self, trace: TraceToSpeedscope.TraceRecording
        ...
        for record, function in enumerate(trace.functions):
            if not trace.stamps[record] & _EXIT_BIT:
                ...
                if trace.functions[first] != function:
                    sys.exit(f"error: record {record}: exit of {function:#x} closes "
    ...
    def document(self, trace: TraceToSpeedscope.TraceRecording,
                 calls: list[TraceToSpeedscope.CallSpan], frames: dict[int, Frame],
                 name: str) -> SpeedscopeDoc:
        ...
        for record in range(first, last + 1):
            ...
            at = round(((stamp & ~_EXIT_BIT) - trace.origin_tsc) / trace.tsc_per_ns)
            events.append({"type": "C" if stamp & _EXIT_BIT else "O", "frame": frame,
                           "at": at})
        ...
        return {"$schema": _SCHEMA,
                ...
                "profiles": [{"type": "evented", "name": profile_name, "unit": "nanoseconds",
                              "startValue": events[0]["at"], "endValue": events[-1]["at"],
                              "events": events}],
    ...
    def frames(self, trace_file: str, functions: list[int]) -> dict[int, Frame]:
        ...
        for function in functions:
            ...
            if mapping is None:
                sys.exit(f"error: {function:#x} is in no executable mapping of "
                         f"{trace_file}.maps")
    ...
    def load(self, trace_file: str) -> TraceToSpeedscope.TraceRecording:
        ...
        _, kept, seen, skip, t0_ns, t0_tsc, t1_ns, t1_tsc = words[:_HEADER_WORDS]
        records = words[_HEADER_WORDS:]
        if len(records) != kept * _RECORD_WORDS or t1_ns <= t0_ns:
            sys.exit(f"error: {trace_file}: truncated, or no time passed between its "
                     "two clock readings")
        return TraceToSpeedscope.TraceRecording(
            seen=seen, skip=skip, origin_tsc=t0_tsc,
    ...
    def mappings_read(self, maps_file: str) -> list[TraceToSpeedscope.ExecutableMapping]:
        ...
        with open(maps_file, encoding="utf-8") as handle:
            for line in handle:
                ...
                if len(fields) == 6 and "x" in fields[1] and fields[5].startswith("/"):
                    ...
                    mappings.append(TraceToSpeedscope.ExecutableMapping(
                        int(low, 16), int(high, 16), int(fields[2], 16), fields[5].strip()))
        return mappings
    ...
    def run(self, args: TraceToSpeedscope.TraceArgs) -> None:
        ...
        if not calls:
            sys.exit(f"error: {args.trace_file}: no call both starts and ends inside "
                     "the kept events")
        frames = self.frames(args.trace_file,
                             sorted({trace.functions[record] for record
                                     in range(calls[0].first, calls[-1].last + 1)}))
        text = ""
        for count in range(1, len(calls) + 1):
            longer = json.dumps(self.document(trace, calls[:count], frames, args.name),
                                separators=(",", ":"))
            if text and len(longer) > _MAX_BYTES:
                break
            text = longer
        if len(text) > _MAX_BYTES:
            print(f"warning: one call alone is {len(text):,} bytes, over the "
                  f"{_MAX_BYTES:,} budget", file=sys.stderr)
    ...
    def segments_read(self, path: str) -> list[TraceToSpeedscope.LoadSegment]:
        listing = subprocess.run(["readelf", "-lW", path], check=True,
                                 capture_output=True, text=True).stdout
        segments: list[TraceToSpeedscope.LoadSegment] = []
        for line in listing.splitlines():
            fields = line.split()
            if fields and fields[0] == "LOAD":
                segments.append(TraceToSpeedscope.LoadSegment(
                    int(fields[1], 16), int(fields[4], 16), int(fields[2], 16)))
        return segments

    # Undo the load address, so addr2line sees the address it has debug info
    # for.
    def virtual_address(self, mapping: TraceToSpeedscope.ExecutableMapping,
                        segments: list[TraceToSpeedscope.LoadSegment],
                        address: int) -> int:
        ...
        sys.exit(f"error: {address:#x} (file offset {offset:#x}) is in no LOAD segment "
                 f"of {mapping.path}")
...
def main() -> None:
    ...
    TraceToSpeedscope().run(TraceToSpeedscope.TraceArgs(
        trace_file=namespace.trace_file, output=namespace.output,
